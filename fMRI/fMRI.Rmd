...
The first model we fit is a [lasso linear model](https://en.wikipedia.org/wiki/Lasso_(statistics)). 
...
The code below extracts the predicted voxel response for the training set.
...
ggplot(prediction.cor) + geom_histogram(aes(x = cor), col = "white", binwidth = 0.02)
```
...
### Visualizing single voxel performance

We now evaluate the performance of the Lasso + OLS model for a single voxel that was randomly selected from among the voxels whose predicted voxel response had correlation at least 0.5 with the true voxel response.

```{r, collapse = TRUE}
set.seed(1348979435)
# randomly select the voxel to view
voxel <- prediction.cor %>% filter(cor > 0.5) %>% 
...
The OLS model for voxel 959 can be written as follows:

$$ \begin{align*}
 \widehat{\text{voxel respnse}}_{959} =  -&0.59 + 0.72 \text{Gabor}^{(959)}_{1} + 1.20 \text{Gabor}^{(959)}_{2} + 1.19 \text{Gabor}^{(959)}_{3} + 0.67 \text{Gabor}^{(959)}_{4}\\
&+ 0.93 \text{Gabor}^{(959)}_{5} + 0.69 \text{Gabor}^{(959)}_{6} + 0.62 \text{Gabor}^{(959)}_{7} + 0.04 \text{Gabor}^{(959)}_{8} \\
&+ 0.71 \text{Gabor}^{(959)}_{9} + 0.98 \text{Gabor}^{(959)}_{10}
\end{align*}$$

So to predict voxel 959's response to an image, you simply need to isolate the 10 selected Gabor wavelet feature values for the image and plug them into the equation above.

```{r, fig.align="center", fig.height = 5, fig.width = 5, cache = TRUE}
# place the predicted and observed responses in a data frame
predictions.df <- data.frame(pred = predictions[[voxel]], 
                             obs = val.resp[, voxel])
# plot observed versus predicted responses
ggplot(predictions.df) + geom_point(aes(x = obs, y = pred)) + 
...
Although the plot above gives us a nice view of how the predicted response compares to the true observed response, it does not give us any indication of how the prediction accuracy for different regions of the data.



```{r voxel959-superheat, cache = TRUE, fig.align="center", fig.height=8, fig.width=8, fig.cap="The central heatmap contains the feature matrix for the validation set filtered to the Gabor features selected by the lasso model for voxel 959. The rows correspond to the validation images, while the columns correspond to the 10 selected Gabor features. The rows/images are ordered based on heirarchical clustering on this matrix. To the right of the heatmap is a scatterplot displaying the voxel 959 response prediction error for each image. Each point is colored based on the observed response of voxel 959 to the corresponding image. Above the heatmap is a barplot displaying the value of the OLS coefficient for each Gabor wavelet feature in the model."}
# extract the OLS coefficient vector for the selected voxel
voxel.features <- selected.features.list[[voxel]]
voxel.beta <- ols.list[[voxel]]$coefficients
# we will color each observation in the right plot based 
# on the observed response size
library(plotrix)
response.size <- color.scale(val.resp[, voxel])

# plot a heatmap of the selected features
png(file = "../../Figures/lasso-validation-vox959.png", 
    height = 1000, width = 800)
superheat(val.feat[, voxel.features],
          yr = predictions[[voxel]] - val.resp[, voxel],
          ...
          yt.plot.type = "bar",
          yt.bar.col = "grey",
          yt.axis.name = "OLS model coefficient",
...
The rows are ordered by the predicted response. Note that the model tends to to overestimate the voxel response in the regions where the Gabor feature values are close to zero (see the rows towards the center of the heatmap).
...
superheat(val.resp, 
          X.text = matrix(c("decreased\nvoxel\nresponse",
                            ...
                            "neutral\nvoxel\nresponse", 
                            "neutral\nvoxel\nresponse"), ncol = 2),
          X.text.col = matrix(c("white", "black", "white", "black"), ncol = 2),
          X.text.size = 8,
          yt = prediction.cor$cor,
          yt.axis.name = "Correlation between\npredicted and true\nvoxel responses",
...
We see a similar group of two voxel clusters where the first group of voxels have correlation close to zero, and the second group has correlation around 0.6.
...
```{r, collapse = TRUE}
rf.prediction.cor %>% filter(voxel == 959)
prediction.cor %>% filter(voxel == 959)
```


```{r, cache = TRUE, fig.align="center", fig.height = 8, fig.width=8}
# plot a heatmap of the selected features
superheat(val.feat[, top.features[[voxel]]],
          yr = rf.predictions[[voxel]] - val.resp[, voxel],
          yr.axis.name = "Voxel response\nprediction error",
          yr.obs.col = response.size,
          yr.plot.size = 0.6,
          
          yt = rf.list[[voxel]]$importance,
          yt.plot.type = "bar",
          yt.bar.col = "grey",
          ...
          title = paste("Random Forest perfomance on the validation set\nimages for voxel", voxel))
```
...
Just like with the Lasso + OLS model, we would like to visualize the performance of the models for all voxels simultaneously. Using the same clusters as for the Lasso + OLS model, we generate the same plot: a heatmap of the *response* matrix, that is, the response of each voxel to each image. Above each column in the heatmap (each column corresponds to a voxel), we plot the correlation of the observed voxel response with the predicted voxel response.

```{r, cache = TRUE, fig.align="center", fig.height = 7, fig.width = 6}
superheat(val.resp, 
          yt = rf.prediction.cor$cor,
          yt.axis.name = "Correlation between\npredicted and true\nvoxel responses",
          yt.obs.col = rep("#66c2a5", ncol(val.resp)),
          yt.point.alpha = 0.6,
          ...
          row.title = "Validation images (1,750)",
          column.title = "Voxels (1,294)")
```
